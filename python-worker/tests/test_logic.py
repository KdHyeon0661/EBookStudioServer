...
# 프롬프트 생성 모듈은 실제 MusicGen 런타임을 불러오므로 경량 테스트 환경에서는 건너뜁니다.
for dependency in ("torch", "transformers", "scipy", "numpy", "nltk"):
    pytest.importorskip(dependency, reason="full music worker dependencies are not installed")
